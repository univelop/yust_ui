...
- `YustFilePicker` and `YustImagePicker` take `enableVirusScanning` instead of `markScanPending`. Same behaviour — a new file starts `pending` instead of with no verdict — but named after the workspace state it reflects rather than after the write it causes.
- `YustImagePicker` shows the indicator on a thumbnail **only when the file is infected**. A grid is read at a glance and its subject is the picture, so a mark on every image buries the one that matters. `YustImageScreen` takes `enableVirusScanning` and shows the full verdict as a labelled banner, which is where "checked and fine" is distinguishable from "never checked".
- Opening, downloading or sharing an infected file asks for confirmation first, via `confirmIfInfected`. It confirms and allows rather than blocking: a verdict can be wrong, the file may be the user's own, and a product that silently refuses to open an attachment teaches people to route around it. A dismissed dialog counts as no. Guards `YustFileHandler.showFile`, `showFileInDefaultApp` and `YustFileHelpers.downloadAndLaunchYustFile`, which is where both pickers' share and multi-select download and the image screen's share all land.
- Bump `yust` to 3.36.0 for `YustFile.virusScanResult`
